...
<div style="position:absolute;width:810px;height:870px;left:0px;top:0px;">
	...
	<div id="cargaForma" style="position:absolute;left:160px;top:364px;width:500px;height:330px;">		
			<div id="forma" style="position:absolute;left:0;top:45px;font-family: Helvetica;font-size: 14px;">
				<div style="position:absolute;left:0;top:0;width:230px;height: 100%;">
					<form id="formaRegistros" name="formaRegistros"  method="POST" action="<?=base_url().'movi_eliminatorias/vista_registro/'?>">									
						<div style="float:left;width:100%;height:30px;margin-top:2px;margin-left:140px;padding-left:20px;">
						...
						<div style="float:left;width:100%;height:30px;margin-left:5px;margin-top:49px;">
...
<script type="text/javascript">	
	...
	var validator = new FormValidator('formaRegistros',rules, function(errors, event) {				
		 if (errors.length > 0) {			
		    	...
		    	$.ajax({  
					  type: "POST",  
					  url:"<?=base_url().'samsung_laberinto/vista_registro/'?>",  
					  data: $('#formaRegistros').serialize(),
					  success: function( response ) {					  					
						  if (response=="1")
						  	$( "#content" ).load("<?=base_url().'samsung_laberinto/liker'?>", { 'user' : JSON.stringify( LibraryFacebook.getUserFbData() ), 'fb_page' : JSON.stringify( LibraryFacebook.getSignedRequest() ) });				 
					  }
					});     
			 }  
	});
