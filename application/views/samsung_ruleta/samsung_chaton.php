...
class Samsung_chaton extends Facebook_Controller
{
    ...
    {
        $this->load->view('samsung_ruleta/indexmobile', $this->data);
    }
    ...
    function alreadyRegistrerRuleta( $table, $where ){
        $this->db->where( $where );
    ...
    function register()
    {
        if (isset($_POST['nombre'])) {
            ...
            if ($this->alreadyRegistrerRuleta('usuarios', array('fbid' => $this->data['user']->id))) {
                $updateUser = array(
                    'nombre' => $_POST['nombre'],
                ...
                $this->db->query("update samsung_ruleta  set id_partido='1',compartidos='3', resultado='$premioNombre' where id_user='$id' or id_user='$fbid'");
    ...
    function ingresoActividad($sw = "0")
    {
        ...
        if (isset($this->data['participante']->id)){
    ...
    function calculaGanador($iduser)
    {
        ...
        if (!$this->modelo->buscarNoRegistrado($iduser)) {
        //    return "pierde";
        }
    ...
    {
        $idParticipante = $_POST ['idParticipante'];
        $query = $this->db->query("update samsung_ruleta set oportunidades=oportunidades + 1  where id_user='$idParticipante'");
