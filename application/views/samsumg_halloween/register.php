...
<div class="contendor-actividad">	
	...
	<div class="registro" >
		...
		<form id="register" name="register" method="post" role="form" class="form-horizontal formulario" >
			...
			<div class="form-group">		
				<div class="form-control inputTexto" style="margin-top: 0px;">
	...
	var rules = [ 
		       ...
		       { name: 'apellido', display: 'apellido', rules: 'required'},
	               ...
	               { name: 'cedula', display: 'cedula', rules: 'required|max_length[10]'},
	...
	var validator = new FormValidator('register',rules, function(errors, event) {		
		...
		$.ajax({  
			  type: "POST",  
			  url: "<?php echo base_url('pronostico/register')?>",  
			  data: $('#'+forma).serialize(),
			  success: function( response ) {
				  if (response=="1"){					  
					$('#submit').hide();
				    $('#content').load("<?php echo base_url('pronostico/ingresoActividad/1')?>",{ 'user': JSON.stringify(LibraryFacebook.getUserFbData()) });
				  }
	    		} 
			}); 
		return false;
