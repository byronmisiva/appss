...
class Samsung_corporate extends CI_Controller{	
	public $data;
	var $controlador;
	var $idApp;
	var $secretApp;
	...
	public function __construct(){
		...
		$this->load->model('mdl_corporate','modelo');
		$this->load->helper('form');
		$this->folderView="corporate";				
		$this->data['controlador']="samsung_corporate";
		$this->data['idApp']="1665122247064113";
		$this->data['secretApp']="5687a6be17aa445a17dfcbdbfa3926e6";		
		$this->data['condiciones'] = "<a href='".base_url()."archivos/corporate/REGLAMENTO DE-TERMINOS-Y-CONDICIONES-PARA-EL-CONCURSO-Corporate.pdf' target='_blank' >Términos y Condiciones</a>";
	}
		...
		if( isset( $_POST['nombre'] ) ){						
			if( $this->usuario_samsung->alreadyRegistrer( 'usuarios', array( 'fbid' => $_POST['fbid'] ) )=="y" ){
				$updateUser = array(
						...
						'mail' => $_POST['mail'],'ultima_app' => "corporate",
				...
				$id = $participante->id;
				$this->db->insert("registro_corporate",array(
								"id_user"=>$id,
								"fbid"=>$_POST['fbid']));				
				$resp="1";
			}
	...
	function verificarParticipante($id){
		...
		if($participante == "0"){
			...
			if ( $registro == FALSE )
				...
				$this->db->update("registro_corporate",array("actividad"=>$actividades));
				echo "A";
			}
		}
	}
	...
	function saveUser(){
		...
		$this->db->update("registro_corporate",array("nivel"=>$valor, "tiempo" => $valorTiempo));
		echo $valor;
	}
	...
	function negativoUser(){
		$participante=$this->modelo->buscarUserFbid($_POST["participante"]);
		$this->db->where("fbid",$_POST["participante"]);
		$this->db->update("registro_corporate",array("nivel"=>"1"));
		echo "1";
	...
	function registrarAmigos($id){
		...
		foreach($arreglo as $row){
			$this->db->insert("corporate_invitados",array("fbid_user"=>$id,"fbid_amigo"=>$row));
		}
		...
		$this->db->update("registro_corporate",array("invitarfb"=>$valor));
	}
	...
	function invitarTwitter($id){	
		...
		$valor=$valor+1;
		$this->db->where("id_user",$participante->id);
		$this->db->update("registro_corporate",array("invitartw"=>$valor));
	}
